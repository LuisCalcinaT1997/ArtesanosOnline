...
if(isset($enviar2)){
	...
	mysqli_query($obj_conexion, "INSERT INTO  productos(name,price,imagen,id_categoria,oferta,descargable) VALUES ('$name', '$price', '$imagen','$categoria','$oferta','$descargable')");
...
<?php 
if(isset($eliminar)){
	mysqli_query($obj_conexion, "DELETE FROM productos WHERE id='$eliminar'");
	redir("?p=agregar_productos");
 
}
?>
<h1>Agregar Productos</h1><br><br>
<form method="post" action="" enctype="multipart/form-data">
	...
	<div class="form-group">
		<select name="oferta" class="form-control">
			...
			<option value="30%">230% de Descuento</option>
			...
			<option value="99%">100% de Descuento</option>
	...
	<div class="form-group"> 
		<button  style="background-color: blue; color: #fff" type="submit" class="btn btn-sucess" name="enviar2"> <i class="fa fa-check"></i> Agregar Producto</button>	
	</div>
</form><br>
<br>
<table class="table table-striped">
	...
	<?php 
		$prod = mysqli_query($obj_conexion, "SELECT * FROM productos ORDER BY id DESC");
		while($rp=mysqli_fetch_array($prod)){
			$preciototal =0;

			$cat=mysqli_query($obj_conexion,"SELECT * FROM categoria WHERE id = '".$rp['id_categoria']."'");
			...
			?>
				<tr>
					...
					<td><?=$preciototal?></td>
					<td><img src="../productos/<?=$rp['imagen']?>" class="img_carro"/></td>
					<td><?=$categoria?></td>
					<td>
