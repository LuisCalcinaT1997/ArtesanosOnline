...
check_admin();
$id=clear($id);
$q=mysqli_query($obj_conexion, "SELECT * FROM productos WHERE id='$id'") or die(mysqli_error($obj_conexion));
...
if(isset($enviarr)){
	...
	$oferta=clear($oferta);
	mysqli_query($obj_conexion, "UPDATE productos SET name='$name',price ='$price', id_categoria='$categoria', oferta ='$oferta' WHERE id= '$id'") or die(mysqli_error($obj_conexion));
...
<form method="post" action="" enctype="multipart/form-data">
		...
		<select name="categoria" required class="form-control">
			...
			<?php 
				...
				while($r=mysqli_fetch_array($q)){
					?>
					<option <?php if($rq['id_categoria'] == $r['id']){?> selected <?php } ?> value="<?=$r['id']?>"><?=$r['categoria']?></option>
					<?php
				}
			?>
		</select>
	...
	<div class="form-group">
		<select name="oferta" class="form-control">
			...
			<option <?php if($rq['oferta']== 30) {echo "selected";}?> value="30%">230% de Descuento</option>
			...
			<option <?php if($rq['oferta']== 99) {echo "selected";}?> value="99%">100% de Descuento</option>

			
		</select>
	</div>
	<div class="form-group"> 
		<button style="background-color: blue; color: #fff" type="submit" class="btn btn-sucess" name="enviarr"> <i class="fa fa-check"></i> Modificar Producto</button>	
	</div>
